...
import {FHE, euint32, euint8, euint256, externalEuint32, externalEuint8, externalEuint256, ebool} from "@fhevm/solidity/lib/FHE.sol";
...
contract SecureKYC is SepoliaConfig {
    ...
    /// @param passportHash Encrypted hash of passport number
    ...
    function submitKYC(
        externalEuint256 passportHash,
        externalEuint32 birthYear,
        externalEuint8 countryCode,
        bytes calldata inputProof
    ) external {
        euint256 encryptedPassport = FHE.fromExternal(passportHash, inputProof);
        ...
        userKYCData[msg.sender] = EncryptedKYCData({
            passportHash: encryptedPassport,
    ...
    function verifyKYC(address user) external onlyAuthorizedVerifier {
        if (!FHE.isInitialized(userKYCData[user].passportHash)) {
            revert InvalidInput();
        }
    ...
    function checkEligibility(address user, bytes32 projectId) external returns (ebool eligible) {
        ...
        if (requirements.requiresPassport) {
            passportEligible = FHE.ne(userKYCData[user].passportHash, FHE.asEuint256(0));
        }
    ...
    /// @param user Address of the user
    /// @return passportHash Encrypted passport hash
    /// @return birthYear Encrypted birth year
    /// @return countryCode Encrypted country code
    function getUserKYCData(address user) external view returns (euint256, euint32, euint8) {
        EncryptedKYCData memory data = userKYCData[user];
        return (data.passportHash, data.birthYear, data.countryCode);
    }
    ...
    function getVerificationStatus(address user) external view returns (bool verified, uint256 timestamp, address verifier) {
        EncryptedKYCData memory data = userKYCData[user];
